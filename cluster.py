...
def hdbscan_cluster(params):
    ...
    pickle.dump(hdb, open(os.path.join(data_dir, params.method + str(params.sample_number) + '.obj'), 'wb'))

    sample_labels = hdb.labels_
    n_clusters_hdb_ = len(set(sample_labels)) - (1 if -1 in sample_labels else 0)
    print('\n\n++ HDBSCAN Results')
    print('Estimated number of clusters: %d' % n_clusters_hdb_)
    with open(params.train_file + params.method + str(params.sample_number), 'w') as file_w:
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'kmeans':
        #加载配置
        config = ConfigParser('./config/kmeans.json') 
        kmeans_cluster(config.model_parameters)
    elif sys.argv[1] == 'hdbscan':
        config = ConfigParser('./config/hdbscan.json') 
        hdbscan_cluster(config.model_parameters)
    else:
        pass
